  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8">
      ...
      {loading ? <div className="space-y-2">{[1,2,3].map(i=><div key={i} className="os-card p-4"><div className="shimmer-bg h-4 w-40 rounded bg-os-elevated"/></div>)}</div>
        ...
        : subs.length === 0 ? <div className="os-card flex min-h-32 items-center justify-center p-4"><p className="text-sm text-os-subtle">暂无智能体提交</p></div>
        : subs.map(s => (
          <div key={s.submission_id} className="os-card os-card-hover mb-2 p-3">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0 flex-1">
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  <Link href={`/developer/agents/${s.submission_id}`} className="min-w-0 flex-1 break-words text-sm font-semibold text-os-text-high hover:text-os-accent">
                ...
                <div className="mt-1 flex flex-wrap gap-1.5 text-2xs text-os-subtle">
                  ...
                  {s.review_notes && <span className="max-w-full break-words text-os-warning">• {s.review_notes}</span>}
                </div>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                ...
                {(s.status === "submitted" || s.status === "in_review") && (
                  <button onClick={() => handleWithdraw(s.submission_id)} disabled={withdrawing.has(s.submission_id)}
                    className="inline-flex h-7 items-center gap-1 rounded border border-os-border px-2 text-xs text-os-subtle hover:border-os-danger/30 hover:text-os-danger disabled:opacity-50">
