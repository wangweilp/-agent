    ...
    <main className="mx-auto max-w-4xl px-4 py-8">
      <header className="mb-6">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-os-border bg-os-surface px-3 py-1 text-xs text-os-subtle">
          <Code2 size={14} className="text-os-accent" /> 开放平台（Open Platform）
        </div>
        <h1 className="text-3xl font-semibold text-os-text-high">开发者控制台</h1>
        <p className="mt-2 text-sm text-os-subtle max-w-xl">构建、提交和跟踪知维 OS 智能体，从 Manifest 草稿进入审核流程。</p>
      </header>
      <section className="os-card flex min-h-48 flex-col items-center justify-center gap-3 text-center p-6">
        ...
        <p className="text-sm text-os-subtle max-w-sm">注册开发者账号后即可创建 API Key、提交智能体 Manifest 并进入审核流程。</p>
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8">
      ...
      <section className="os-card mb-4 p-4">
        <h3 className="text-sm font-semibold text-os-text-high">开发者资料</h3>
        ...
        <p className="mt-2 break-all font-mono text-2xs text-os-subtle">{dev.developer_id}</p>
      </section>

      {/* Stats */}
      <section className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-6">
        <Stat icon={Key} label="API Key" value={String(keyCount)} />
        <Stat icon={Layers} label="提交总数" value={String(stats.total)} />
        <Stat icon={Sparkles} label="草稿" value={String(stats.draft)} />
        <Stat icon={Clock} label="审核中" value={String(stats.submitted)} />
        <Stat icon={CheckCircle2} label="已通过" value={String(stats.approved)} />
        <Stat icon={XCircle} label="已驳回" value={String(stats.rejected)} />
      </section>
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Link href="/developer/api-keys" className="os-card os-card-hover flex items-center gap-4 p-4">
          ...
          <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-os-text-high">API Key</p><p className="text-xs text-os-subtle">管理 API Key</p></div>
        ...
        <Link href="/developer/agents" className="os-card os-card-hover flex items-center gap-4 p-4">
          ...
          <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-os-text-high">智能体提交</p><p className="text-xs text-os-subtle">查看和管理提交</p></div>
          <ArrowRight size={14} className="shrink-0 text-os-muted" />
        </Link>
        <Link href="/developer/agents/new" className="os-card os-card-hover flex items-center gap-4 p-4">
          ...
          <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-os-text-high">新建智能体提交</p><p className="text-xs text-os-subtle">提交新智能体</p></div>
          <ArrowRight size={14} className="shrink-0 text-os-muted" />
        </Link>
      </section>
      ...
      <section className="os-card mt-4 p-4">
        <h3 className="flex items-center gap-2 text-xs font-semibold text-os-subtle"><Shield size={13} className="text-os-accent"/> 开放平台边界说明</h3>
        <ul className="mt-2 space-y-1 text-2xs text-os-subtle">
          <li>• 开发者控制台用于创建和提交智能体 Manifest</li>
...
function Info({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0"><p className="text-2xs text-os-subtle">{label}</p><p className="mt-0.5 break-words text-xs text-os-text-high">{value}</p></div>;
}
